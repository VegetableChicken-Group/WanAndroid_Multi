...
interface IAccountService : IProvider, CookieJar {
   ...
   * 在 ViewModel 中也推荐使用 [observeUserInfoFlow]，或者使用 ViewModel 中的 Observable#lifecycle() 保证生命周期
   ...
   * 3、如果你想对于不同学号返回给下游不同的 Flow，
   * 操作符为 switchMap，但目前该操作符被废弃，新操作符 flatMapLatest 还在测验阶段，
   * 所以更推荐使用 [observeUserInfo] 的 [Observable] 来处理
   *
   * 回调的数据为 null 时，说明退出了登录
   */
