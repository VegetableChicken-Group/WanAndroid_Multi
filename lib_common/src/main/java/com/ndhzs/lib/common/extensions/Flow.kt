 ...
 * 4、顺便说一下数据流动的原则，
 *    个人观点：
 *    1、推荐 Room 使用 Observable 来观察本地数据
 *    2、Room 需要的数据类强烈不建议直接使用网络请求的 Bean 类，因为这样以后不好维护
 *    3、Repository 层用于转换 网络请求的 Bean 数据为 Room 的数据类，然后统一暴露 Room 数据类给 ViewModel
 *       而不是暴露 网络请求的 Bean 类
 *    4、如果网络请求数据简单，不需要本地化，可以省去 Repository 层
 */
