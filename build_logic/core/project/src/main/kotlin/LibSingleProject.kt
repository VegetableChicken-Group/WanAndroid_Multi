 ...
 *
 * @author 985892345
 * @date 2024/2/15 16:29
 */
class LibSingleProject(project: Project) : BaseLibraryProject(project) {
  override fun initProject() {
    implementationApiParentModule()
  }

  override fun initAndroid(extension: LibraryExtension) {
   ...
   */
  private fun implementationApiParentModule() {
    // 通过当前 gradle 运行参数拿到执行单模块调试的 project
    val singleModuleEntryProject = gradle.startParameter
      .taskNames
      .singleOrNull()
      ?.let {
        if (it.endsWith(":assembleDebug")) {
          project(it.substringBefore(":assembleDebug"))
        } else null
      } ?: return
