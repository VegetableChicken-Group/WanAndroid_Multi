...
object ModuleDebugProject : BaseApplicationProject() {
   ...
   *
   * 但存在 单模块A 依赖了 模块B，模块B 依赖了 api模块C，这种间接依赖 api 模块医药进行处理，不然也会报空指针
   *
   * 所以该方法就是为了进行递归遍历整个依赖图(数据结构中的图)，采取的深度遍历，
   * 将所有间接或直接的 api 依赖都附上该 api 的实现模块
   *
                ...
                ApiDependUtils.sApiWithImplMap[apiPath]?.dependApiImplOnly(debugProject) {
                  (!dependPathSet.contains(it) && it != debugProject.path).apply {
                    if (this) {
                      ...
                      * 所以会比下面紧接着的 forEach 调用前再引入刚才的新依赖，所以需要在这里 add
                      * */
