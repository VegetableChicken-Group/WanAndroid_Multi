...
**注：** 由于 ARouter 停止维护，现替换为我自己写的 KtProvider: https://github.com/985892345/KtProvider
...
> 
> **注**：现已不再使用 ARouter，下面的 ServiceManager 为旧版写法，新版已替换为 KtProvider
>
>```kotlin
>/**
...
4. 配置 debug 下的 `AndroidManifest`
   ...
   > 单独创建一个新的 `AndroidManifest` 在 debug 包下
   > 
   > 下面代码中有个 `DebugApp`，是为了自定义初始化加载，**因为使用 KtProvider 后，目前必须新建一个 \*App 继承于 `BaseApp`**
   >
   ...
   > **注意：**
   >
   > - `android:name="debug.app.DebugApp"`，因为使用 KtProvider 后，目前必须新建一个 \*App 继承于 BaseApp
...
### XML预览主题问题
由于多模块的原因，XML 不能很好的预览，所以需要你单独设置主题
