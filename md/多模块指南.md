...
1. 这里选择 `Phone & Tablet`，**记得把那个下划线改成点号**，还有后面写 `Activity` 名字，别只会写 `MainActivity`！还有 `layout_main` 文件
...
2. 创建好了后会与[创建 `lib` 模块](#新建lib模块)一样报错，改法也是一样的，这里不再叙述，点击跳转：[新建模块的注意事项](#新建模块的注意事项)
...
4. 在需要得到该接口的模块中引入该 `api` 模块
   ...
   // 所以在项目种强烈不建议使用上面的写法，为了统一写法，我在插件中进行了配置
   // 应该改为:
   
   // build_logic 模块中 /api/ApiDepend.kt 中：
   object ApiDepend {
     // 写上你的 api 模块路径
   ...
   // build_logic 模块中 /api/ApiDepend.kt 中：
   // 在下面增加一个函数，其中 extraImplDepend 是依赖除父模块外的额外实现模块，一般用不到
...
2. 设置 `debug` 文件夹
   ...
   >- `android:name="debug.app.DebugApp"` 
   >- `android:label="module_test"` 改一下打包后的名字
...
1. 先解除 `settings.gradle.kts` 对模块的依赖
