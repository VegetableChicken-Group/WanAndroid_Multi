...
1. 选择 `Android Library`，**记得把那个下划线改成点号**
   ...
   <img src="C:\Users\GuoXiangrui\Desktop\image-20220528004325678.png" alt="image-20220528004325678" style="zoom:50%;" />
...
3. 删除模块目录下的 `consumer-rules.pro`、`proguard-rules.pro`、`libs`
   ...
   <img src="C:\Users\GuoXiangrui\Desktop\image-20220528004551563.png" alt="image-20220528004551563" style="zoom:50%;" />
...
2. 创建好了后需要与[创建 `lib` 模块](#新建lib模块)一样修改一遍，这里不再叙述，点击跳转：[新建模块的注意事项](#新建模块的注意事项)
   ...
   >```
   >
   >注意：其中 `dependAndroidView()`、`dependAndroidKtx()`、`dependLifecycleKtx()` 已给 module 模块默认依赖，其他模块需要依赖时，请按需选择依赖，而不是依赖全部
...
9. 由于 AS 的原因，每次按照它的模板新建 `Activity` 和 `Fragment`，它都会主动添加 `android` 插件和默认依赖，如下：
   ...
   > 注意：图中`dependAndroidView()`、`dependAndroidKtx()`、`dependLifecycleKtx()` 已给 module 模块默认依赖，所以不用再写，图为老图，就没改了
...
![image-20220719120146520](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora-after-22-7-19/image-20220719120146520.png)
...
**在根目录的 `.gitignore` 中的 `/build` 改为 `build`**，就可以了，但是有个小缺点，就是**所有包名为 build 都会被忽略提交**，以后避免写包名为 build 就可以了
