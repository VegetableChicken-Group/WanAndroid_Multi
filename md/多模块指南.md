...
2. 选择模块模板，这里有多个选项
   ...
   ![image-20220527182133853](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220527182133853.png)
...
1. 选择 `Android Library`，**记得把那个下划线改成点号**
   ...
   ![image-20220528004325678](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220528004325678.png)
...
2. 修改 `build.gradle`，由于项目中自定义了模块的管理（`build_logic`），所以使用 AS 新建模块需要修改一些东西
   ...
   >建好后一定会报这个错误：
   >
   >![image-20220527183444752](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220527183444752.png)
   >
   >下面是修改教程：
   >
   >1. 先打开**根项目**下的 `build.gradle.kts`，把下图圈的 `buildscript` 全部删掉，每次新建模块他都会自动添加这个东西，原因是使用了 `build_logic` 来管理模块后，就不用再设置这个 `classpath`
   >
   >   ![image-20220527195657454](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220527195657454.png)
   >
   >   
   >
   >2. 打开根目录下的 `settings.gradle.kts`，把第三行的 `include(":lib_account")` 给删掉
   >
   >   原因在于我修改了 `settings.gradle.kts` 的 `include` 逻辑，直接采取遍历目录来自动 `include`，所以需要删掉 AS 自己添加的 `include(":lib_account")`
   >
   >   
   >
   >3. 打开新建的模块，修改它的 `build.gradle.kts`，基本上是全部删掉，然后改成下面这样
   >
   >   ```kotlin
...
3. 删除模块目录下的 `consumer-rules.pro`、`proguard-rules.pro`、`libs`
   ...
   ![image-20220528004551563](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220528004551563.png)
...
1. 这里选择 `Phone & Tablet`，**记得把那个下划线改成点号**，还有后面写 `Activity` 名字，别只会写 `MainActivity`！还有 `layout_main` 文件记得改成 `模块名_layout_xxx`

   ![image-20220527202143501](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220527202143501.png)

   

2. 创建好了后会与[创建 lib 模块](#新建 lib 模块)一样报错，改法也是一样的，这里不再叙述，点击跳转：[新建模块的注意事项](#新建模块的注意事项)

   

3. 删除模块目录下的 `proguard-rules.pro`、`libs`

   ![image-20220527203356524](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220527203356524.png)

   

4. 删除 `res` 下的这些东西，删除时可能会告诉你有引用持有，不管，直接 `Delete Anyway`

   ![image-20220527203844369](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220527203844369.png)
...
1. 需要向外提供接口的 `module` 模块下新建模块
   ...
   ![image-20220527205938656](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220527205938656.png)

   

2. 其余过程与[新建 `lib` 模块]($新建 lib 模块)基本一样
...
如果不这样写，那么在 `Activity` 被系统回收在重建时（此时 `Fragment` 同样被回收），会重新走 `onCreate()` 流程，但是，`Fragment` 被回收后是会**被系统重新创建的**（而且还自动帮你 `replace() 回到原状态`），此时你因为走了 `onCreate()` 流程，就会导致出现两个同类型的 `Fragment`，你新创建的 `Fragment` 调用 `replace()` 会替换掉系统帮你还原的 `Fragment`
...
2. 建一个接口 `ITestService`
   ...
   >   */
   >  fun startTestActivity(data: Data)
   >  
   >  /**
   >   * 返回一个数据
   >   */
...
3. 在 `module_test` 中依赖 `api_test` 模块，并实现 `ITestService`
   ...
   > **注：**由于我在 `build_logic` 中默认设置了外模块自动依赖自己的子模块，所以可以不用写下面这步
   >
   > ```kotlin
   ...
   >   
   >   override fun startTestActivity(data: ITestService.Data) {
   >     mDataLive.postValue(data)
   >     TestActivity.start(mContext, data)
   >   }
   >   
...
2. 设置 `debug` 文件夹
   ...
   >![image-20220528141602069](https://img-1307243988.cos.ap-chengdu.myqcloud.com/typora/image-20220528141602069.png)
   >
   >增加 `DebugApp` 是为了继承 `BaseApp` 来实现自己所需要的一些初始化加载
   >
